...
public abstract class LiveBaseActivity extends BaseActivity {
    ...
    private void showPayMentTip(final RoomGiftListDialog dialog, final int id) {
        ...
        if (PreferenceManager.getInstance().getPayMentTip()) {
            ...
            builder.setTitle("提示")
                    .setMessage("该礼物需要支付" + gift.getGprice() + ",你确认支付吗？")
                    .setView(view)
    ...
    public void sendGift(final RoomGiftListDialog dialog, final Gift gift) {
        long change = PreferenceManager.getInstance().getCurrentuserChange();
        L.e(TAG, "gift.price=" + gift.getGprice());
        L.e(TAG, "change=" + change);
        if (change >= gift.getGprice()) {
            NetDao.givindGift(LiveBaseActivity.this, EMClient.getInstance().getCurrentUser(), chatroom.getOwner(), gift.getId(), 1, new OnCompleteListener<String>() {
                @Override
    ...
    private void sentGiftMsg(RoomGiftListDialog dialog, int id) {
        User user = EaseUserUtils.getAppUserInfo(EMClient.getInstance().getCurrentUser());
